...
| 编译 | `cmake --build <build> --parallel <min(逻辑 CPU, 16)>` | 相同 |
...
脚本内部自动把并发编译数设为 `min(逻辑 CPU 数, 16)`，并在开始时打印最终数字；用户不需要设置线程参数。编译阶段的多行输出可能交错，这是多个并行编译进程同时输出日志的正常现象，不代表脚本启动了后台传输或其他异步任务。
